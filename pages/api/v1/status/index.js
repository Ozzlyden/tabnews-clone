...
async function status(request, response) {
  ...
  const databaseMaxConnectionResult = await database.query("SHOW max_connections;"); // Mostra no número maximo de conexões do BD
  const databaseMaxConnectionValue = databaseMaxConnectionResult.rows[0].max_connections; // Por padrão geralmente são 100
